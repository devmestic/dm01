...
def key_parts(key):
    if '#' in key:
        ...
        return base,selector_norm(suffix)
    if '$' in key:
        base,layer=key.split('$',1)
        return base,'$'+selector_norm(layer)
    return key,''

def base_aliases(base):
    out=[base]
    # Story data carries this historical compound alias; current bundle object uses fmzuki.
    if 'weiywfmzuki' in base.lower():
        out.append(re.sub('weiywfmzuki','fmzuki',base,flags=re.I))
    return list(dict.fromkeys(out))

def bundle_stems(key):
    base,_=key_parts(key)
    out=[]
    for b in base_aliases(base):
        out.append(b)
        if b.endswith('_1'):out.append(b[:-2])
        else:out.append(b+'_1')
    return list(dict.fromkeys(out))

def candidates(key,names):
    base,suffix=key_parts(key)
    low={n.lower():n for n in names if '[alpha]' not in n.lower()}
    names=list(low.values())
    guesses=[]
    def add(x):
    ...
    for b in base_aliases(base):
        if suffix:
            if suffix.startswith('$'):
                layer=suffix[1:]
                add(b+'$'+layer);add(b+'_'+layer);add(b+'_1$'+layer)
            else:
                add(b+'#'+suffix);add(b+'_1#'+suffix);add(suffix)
                if '$' in suffix:
                    expr,layer=suffix.split('$',1)
                    add(expr+'$'+layer);add(b+'$'+layer);add(b+'_1$'+layer);add(b+'_1#'+expr+'$'+layer)
            add(b+'$1');add(b+'_1');add(b+'_1#1');add(b+'#1');add(b)
        else:
            add(b);add(b+'_1');add(b+'#1');add(b+'_1#1');add(b+'$1')
    for g in guesses:
        if g.lower() in low:return low[g.lower()],guesses,'exact-or-alias'
    # Default/base pose convention in old AVG bundles.
    defaults=[]
    for n in names:
        nl=n.lower()
        if nl.endswith('_1') or nl.endswith('#1') or nl.endswith('_1#1') or nl.endswith('$1'):
            defaults.append(n)
    if defaults and (not suffix or suffix in ('1','$1')):
        return sorted(defaults,key=lambda x:(len(x),x.lower()))[0],guesses,'default-pose'
    # Mayer's current bundle has expressions #2..#5 while old story data still calls #1.
    # Use the first actual sprite only for a missing default-expression selector, never for arbitrary expressions.
    if suffix=='1' and names:
        ordered=sorted(names,key=lambda x:(0 if '#2' in x else 1,len(x),x.lower()))
        return ordered[0],guesses,'legacy-default-fallback'
    if len(names)==1:return names[0],guesses,'single-sprite'
    return None,guesses,None
...
for key in missing:
    bundle=None; tried=[]
    for stem in bundle_stems(key):
        wanted=('avg/characters/'+stem+'.ab').lower();tried.append(wanted)
        if wanted in manifest:
            bundle=manifest[wanted];break
    if bundle:bybundle.setdefault(bundle,[]).append(key)
    else:no_bundle[key]=tried

report={'resVersion':res,'missingBefore':len(missing),'bundles':len(bybundle),'recovered':{},'unresolved':{},'noBundle':no_bundle,'bundleErrors':[]}
    ...
    for i,(bundle,keys) in enumerate(sorted(bybundle.items()),1):
        ...
        try:
            ...
            for ab in unpack.rglob('*.ab'):
                env=UnityPy.load(str(ab));objs=list(env.objects)
                for pass_type in ('Sprite','Texture2D'):
                    for obj in objs:
                        ...
                        if not name or '[alpha]' in name.lower() or name.lower() in render:continue
                        try:img=getattr(data,'image',None)
                        except Exception:img=None
                        if img is not None:render[name.lower()]=(name,img)
            names=[v[0] for v in render.values()]
            for key in keys:
                chosen,guesses,mode=candidates(key,names)
                if not chosen:
                    report['unresolved'][key]={'bundle':bundle,'objects':names[:80],'guesses':guesses};continue
                try:rel=save_pil(render[chosen.lower()][1],key)
                except Exception as e:
                    report['unresolved'][key]={'bundle':bundle,'chosen':chosen,'error':repr(e)};continue
                chars[key]=rel
                report['recovered'][key]={'bundle':bundle,'object':chosen,'mode':mode,'asset':rel}
                print('RECOVERED_OFFICIAL_CHARACTER',key,'<-',bundle,'::',chosen,'mode',mode)
        except Exception as e:
...
print(json.dumps({'resVersion':res,'requested':len(requested),'found':found,'missing':len(requested)-found,'recovered':len(report['recovered']),'unresolved':len(report['unresolved']),'noBundle':len(report['noBundle']),'bundleErrors':len(report['bundleErrors'])},ensure_ascii=False,indent=2))
